...
export class UserController {
  ...
  @UseGuards(
    VerifyToken,
    RoleGuard(Roles.Admin),
    PermissionGuard(Permission.CreateUser),
  )
  @ApiResponse({
    status: 201,
  ...
  @UseGuards(
    VerifyToken,
    RoleGuard(Roles.Admin),
    PermissionGuard(Permission.UpdateUser),
  )
  @ApiParam({
    name: 'id',
    required: true,
    description: 'update folow id user',
  ...
  @UseGuards(
    VerifyToken,
    RoleGuard(Roles.Admin),
    PermissionGuard(Permission.DeleteUser),
  )
  @ApiParam({
    name: 'id',
    required: true,
    description: 'delete folow id user',
    schema: { oneOf: [{ type: 'string' }, { type: 'integer' }] },
  })
